...
// 变量在声明的时候如果有初始值，我们可以使用 := 操作符来简短声明： 声明变量的简短语法是 name := initialvalue，一个短变量声明操作符在一次操作中完成两件事儿：声明一个变量，并初始化。
// 简短声明只能用在函数中, 不能放到函数外部
func variableShortHand() {
	...
	bb, cc := 40, 50 // b已经声明，但c尚未声明
	fmt.Println("bb is", b, "cc is", cc)
	bb, cc = 80, 90 // 给已经声明的变量b和c赋新值
	fmt.Println("changed bb is", bb, "cc is", cc)
	// 上面的程序中，在 bb, cc := 40, 50 这一行，虽然变量 bb 在之前已经被声明了，但是 cc 却是新声明的变量，因此这是合法的
}
